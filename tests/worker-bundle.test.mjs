...
test("Worker output excludes browser-only preview copies", async () => {
  const serverFiles = await walk(serverRoot);
  const serverWebpFiles = serverFiles.filter((file) => file.toLowerCase().endsWith(".webp"));
  const serverBytes = (
    await Promise.all(serverFiles.map(async (file) => (await stat(file)).size))
  ).reduce((total, size) => total + size, 0);

  assert.deepEqual(serverWebpFiles, []);
