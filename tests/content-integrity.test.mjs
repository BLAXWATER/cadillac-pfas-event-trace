...
test("timeline source and preview assets are present", async () => {
  ...
  for (const reference of helperReferences) {
    const pathname = decodeURIComponent(new URL(reference, "https://local.invalid").pathname);
    const publicPath = pathname.includes("/public/") ? pathname.slice(pathname.indexOf("/public/") + 7) : pathname;
    const manifestPreview = firstPagePreviewManifest[publicPath];
    if (manifestPreview) {
...
test("corpus audit reconciles prior OCR coverage and explicitly bounded new-page review", async () => {
  ...
  assert.equal(audit.stats.pdfRecords, 1465);
  assert.equal(audit.stats.pdfPages, 25494);
...
test("compliance archive audit preserves distinct records and excludes verified overlaps", async () => {
  ...
  assert.equal(audit.stats.sourceFilesReviewed, 120);
  assert.equal(audit.stats.finalDistinctRecords, 65);
  ...
  assert.equal(audit.stats.reviewedPages, 798);
  assert.equal(audit.stats.ocrPages, 327);
...
test("correspondence archive audit preserves distinct records and reuses exact cross-category copies", async () => {
  ...
  assert.equal(audit.stats.sourceFilesReviewed, 53);
  assert.equal(audit.stats.reviewedPages, 250);
  assert.equal(audit.stats.sourceOcrPages, 23);
  assert.equal(audit.stats.finalDistinctRecords, 44);
  assert.equal(catalog.length, audit.stats.finalDistinctRecords);
...
test("Wexford archive preserves prior OCR audit and explicitly bounded new-plan review", async () => {
  ...
  assert.equal(audit.stats.sourcePagesAndImagesReviewed, 1779);
...
test("batch 85 adds the February 2021 Wexford packet and reuses Construction Permit 4127", async () => {
  const audit = JSON.parse(await readFile(path.join(appDirectory, "batch85-wexford-monitoring-correspondence-intake-audit.json"), "utf8"));
  ...
  assert.equal(audit.stats.receivedPages, 91);
  assert.equal(audit.stats.gpuOcrPages, 40);
  assert.equal(audit.stats.gpuOcrPagesWithText, 40);
  assert.equal(audit.stats.recordsAdded, 1);
  assert.equal(audit.stats.existingRecordsReusedAsCrossReferences, 1);
  assert.equal(audit.stats.timelineEventsAdded, 1);
  assert.equal(audit.stats.evidenceRequirementsClosed, 0);

  const duplicate = audit.inputs.find((item) => item.classification.includes("cross-reference"));
  const duplicateCatalog = catalogs.get(duplicate.canonicalCatalog);
  const duplicateMatches = duplicateCatalog.filter((item) => item.sha256 === duplicate.sha256);
  assert.equal(duplicateMatches.length, 1);
  assert.equal(duplicateMatches[0].id, duplicate.canonicalRecordId);

  const added = audit.inputs.find((item) => item.recordId);
  const addedCatalog = catalogs.get(added.catalog);
  const addedMatches = addedCatalog.filter((item) => item.sha256 === added.sha256);
  assert.equal(addedMatches.length, 1);
  assert.equal(addedMatches[0].id, added.recordId);
  assert.equal(addedMatches[0].url, added.asset);
  assert.equal(addedMatches[0].pages, added.pages);
  assert.equal(addedMatches[0].size, added.size);
  const bytes = await readFile(path.join(publicDirectory, added.asset.replace(/^\//, "")));
  assert.equal(bytes.length, added.size);
  assert.equal(createHash("sha256").update(bytes).digest("hex"), added.sha256);
  assert.ok(previewManifest[added.asset]);

  const placed = placement.records.find((item) => item.id === added.recordId);
  assert.ok(placed);
  assert.equal(placed.archiveId, "wexford");
  assert.match(pageSource, /EGLE accepts Wexford's 2020 long-term monitoring report/);
  assert.match(pageSource, /January 22 agency-letter date/);
  assert.match(audit.evidentiaryBoundary, /does not include the underlying 2020 monitoring data/i);
});

test("batch 86 adds two audited Cadillac financial reports and bounded DEHP context", async () => {
  const audit = JSON.parse(await readFile(path.join(appDirectory, "batch86-financial-and-dehp-intake-audit.json"), "utf8"));
  const catalogs = await loadCatalogs();
  ...
  assert.equal(audit.stats.suppliedFiles, 11);
  assert.equal(audit.stats.distinctInputHashes, 9);
  assert.equal(audit.stats.pdfPageInstancesReviewed, 1150);
  assert.equal(audit.stats.exactExistingRecordsReusedAsCrossReferences, 6);
  assert.equal(audit.stats.sameBatchDuplicateCopiesSuppressed, 2);
  assert.equal(audit.stats.recordsAdded, 3);
  assert.equal(audit.stats.addedPdfPages, 385);
  assert.equal(audit.stats.addedPagesWithEmbeddedText, 370);
  assert.equal(audit.stats.addedSparsePagesOCRCheckedOnGPU, 15);
  ...
  for (const added of audit.addedRecords) {
    const matches = records.filter((row) => row.sha256 === added.sha256);
    assert.equal(matches.length, 1);
    assert.equal(matches[0].id, added.id);
    assert.equal(matches[0].pages, added.pages);
    assert.equal(matches[0].archive, "supplemental-documents.json");
    const bytes = await readFile(path.join(publicDirectory, matches[0].url.replace(/^\//, "")));
    assert.equal(bytes.length, matches[0].size);
    assert.equal(createHash("sha256").update(bytes).digest("hex"), added.sha256);
    assert.ok(previewManifest[matches[0].url]);
    assert.equal(placement.records.find((row) => row.id === added.id)?.archiveId, "supplemental");
  }

  for (const reused of audit.exactExistingRecordsReused) {
    const matches = records.filter((row) => row.sha256 === reused.sha256);
    assert.equal(matches.length, 1);
    assert.equal(matches[0].id, reused.catalogId);
  }

  const financialEvents = JSON.parse(await readFile(path.join(appDirectory, "leachate-finance-findings.json"), "utf8")).events;
  assert.match(pageSource, /leachateFinance.events.map/);
  for (const year of ["2016", "2017", "2018"]) {
    const event = financialEvents.find(row => row.year === year && row.title.startsWith("City reports $"));
    assert.ok(event);
    assert.match(event.significance, /limited auditor procedures, not an audit opinion/);
    assert.ok(event.sources.some(source => source.page === 25));
  }
...
test("August 28 archive intake distinguishes exact copies from evidentiary exclusions", async () => {
  ...
  assert.equal(supplementalCatalog.length, 195);
...
test("batch 62 reuses, corrects and localizes the exact FY19 biosolids laboratory package", async () => {
  const audit = JSON.parse(await readFile(path.join(appDirectory, "batch62-fy19-metals-nutrients-recheck-audit.json"), "utf8"));
  const catalog = JSON.parse(await readFile(path.join(appDirectory, "lab-documents.json"), "utf8"));
  const previewManifest = JSON.parse(await readFile(path.join(appDirectory, "first-page-preview-manifest.json"), "utf8"));
  const placement = JSON.parse(await readFile(path.join(publicDirectory, "record-placement-manifest.json"), "utf8"));
  ...
  assert.equal(audit.stats.exactPublishedAssetHashMatches, 1);
  assert.equal(audit.stats.recordsLocalized, 1);
  assert.equal(audit.stats.recordYearsCorrected, 1);
  assert.equal(audit.stats.recordDescriptionsExpanded, 1);
  assert.equal(audit.stats.recordsAdded, 0);
  assert.equal(audit.stats.timelineEventsAdded, 0);
  assert.equal(audit.stats.evidenceRequirementsClosed, 0);
  ...
  assert.match(row.description, /Arsenic was reported at 61\.4 mg\/kg/i);
  assert.match(row.description, /contains no PFAS analytes or PFAS-specific method/i);
  assert.equal(previewManifest[expected.asset], expected.preview);
...
test("batch 63 reuses and localizes the exact Wexford site-location figure", async () => {
  const audit = JSON.parse(await readFile(path.join(appDirectory, "batch63-wexford-site-location-recheck-audit.json"), "utf8"));
  const catalog = JSON.parse(await readFile(path.join(appDirectory, "wexford-documents.json"), "utf8"));
  const previewManifest = JSON.parse(await readFile(path.join(appDirectory, "first-page-preview-manifest.json"), "utf8"));
  const placement = JSON.parse(await readFile(path.join(publicDirectory, "record-placement-manifest.json"), "utf8"));
  ...
  assert.equal(row.size, expected.size);
  assert.equal(row.pages, expected.pages);
  assert.match(row.description, /dated February 20, 2012/i);
  assert.match(row.description, /regional orientation only/i);
  assert.equal(previewManifest[expected.asset], expected.preview);
...
test("batch 66 preserves the native CSI email, original workbook and official readable rendition without inventing a PFAS finding", async () => {
  const audit = JSON.parse(await readFile(path.join(appDirectory, "batch66-csi-email-workbook-audit.json"), "utf8"));
  const catalogs = new Map((await loadCatalogs()).map((catalog) => [catalog.name, catalog.rows]));
  const previewManifest = JSON.parse(await readFile(path.join(appDirectory, "first-page-preview-manifest.json"), "utf8"));
  const placement = JSON.parse(await readFile(path.join(publicDirectory, "record-placement-manifest.json"), "utf8"));

  assert.equal(audit.stats.receivedFiles, 1);
  assert.equal(audit.stats.extractedAttachments, 2);
  assert.equal(audit.stats.workbookSheetsReviewed, 8);
  assert.equal(audit.stats.workbookFormulaErrorsPreserved, 1);
  assert.equal(audit.stats.officialPdfPagesReviewed, 18);
  assert.equal(audit.stats.officialPdfEmbeddedTextPages, 9);
  assert.equal(audit.stats.officialPdfSparsePagesOCRChecked, 9);
  assert.equal(audit.stats.renderedPdfPagesReviewed, 18);
  assert.equal(audit.stats.recordsAdded, 3);
  assert.equal(audit.stats.embeddedSignatureImagesSuppressed, 1);
  assert.equal(audit.stats.timelineEventsAdded, 0);
  assert.equal(audit.stats.evidenceRequirementsClosed, 0);
  assert.equal(audit.stats.hashFailures, 0);
  assert.equal(audit.stats.unreadablePages, 0);
  ...
  const pdfRecord = catalogs.get("compliance-documents.json").find((item) => item.id === "064-8ccde0a7d5d7");
  assert.equal(pdfRecord.pages, 18);
  assert.match(pdfRecord.description, /no PFAS analytes/i);
  const pdfExpected = audit.addedRecords.find((item) => item.recordId === pdfRecord.id);
  assert.equal(previewManifest[pdfExpected.asset], pdfExpected.preview);
  const previewBytes = await readFile(path.join(publicDirectory, pdfExpected.preview.replace(/^\//, "")));
  assert.equal(createHash("sha256").update(previewBytes).digest("hex"), path.basename(pdfExpected.preview, ".webp"));

  const messageBytes = await readFile(path.join(publicDirectory, "correspondence-docs", "corr-043-dafd413abaeb.msg"));
  assert.equal(messageBytes.subarray(0, 8).toString("hex"), "d0cf11e0a1b11ae1");
  const workbookBytes = await readFile(path.join(publicDirectory, "reference-data", "126-5811dc8c913b.xlsx"));
...
test("batch 68 reconciles the Cadillac Septage and SIU filenames to exact existing records", async () => {
  const audit = JSON.parse(await readFile(path.join(appDirectory, "batch68-cadillac-septage-siu-reconciliation-audit.json"), "utf8"));
  const catalogs = new Map((await loadCatalogs()).map((catalog) => [catalog.name, catalog.rows]));
  const previewManifest = JSON.parse(await readFile(path.join(appDirectory, "first-page-preview-manifest.json"), "utf8"));
  const placement = JSON.parse(await readFile(path.join(publicDirectory, "record-placement-manifest.json"), "utf8"));
  ...
  assert.equal(audit.stats.sameNamedCopiesHashChecked, 24);
  assert.equal(audit.stats.distinctResolvedHashes, 2);
  assert.equal(audit.stats.exactExistingRecordsReusedAsCrossReferences, 2);
  assert.equal(audit.stats.pdfPagesRenderedAndReviewed, 27);
  assert.equal(audit.stats.recordsAdded, 0);
  assert.equal(audit.stats.timelineEventsAdded, 0);
  assert.equal(audit.stats.evidenceRequirementsClosed, 0);
  assert.equal(audit.stats.hashFailures, 0);
  ...
  for (const expected of audit.exactCrossReferences) {
    ...
    assert.equal(placed.length, 1);
    assert.equal(placed[0].url, expected.asset);
    const preview = previewManifest[expected.asset];
    const previewBytes = await readFile(path.join(publicDirectory, preview.replace(/^\//, "")));
    assert.equal(createHash("sha256").update(previewBytes).digest("hex"), path.basename(preview, ".webp"));
  }

  assert.match(audit.findings.join(" "), /Neither record is a PFAS analytical result/i);
  assert.deepEqual(audit.queueResolution.closedRequirementIds, []);
});
...
test("batch 71 reuses the exact injection-well resolution while preserving its draft status", async () => {
  const audit = JSON.parse(await readFile(path.join(appDirectory, "batch71-injection-well-resolution-reconciliation-audit.json"), "utf8"));
  const catalogs = new Map((await loadCatalogs()).map((catalog) => [catalog.name, catalog.rows]));
  const previewManifest = JSON.parse(await readFile(path.join(appDirectory, "first-page-preview-manifest.json"), "utf8"));
  const placement = JSON.parse(await readFile(path.join(publicDirectory, "record-placement-manifest.json"), "utf8"));
  ...
  assert.equal(audit.stats.recoveredMatchingInputs, 1);
  assert.equal(audit.stats.distinctInputHashes, 1);
  assert.equal(audit.stats.pdfPagesRenderedAndReviewed, 5);
  assert.equal(audit.stats.exactExistingRecordsReusedAsCrossReferences, 1);
  assert.equal(audit.stats.recordsAdded, 0);
  assert.equal(audit.stats.timelineEventsAdded, 0);
  assert.equal(audit.stats.evidenceRequirementsClosed, 0);
  ...
  assert.equal(placement.records.filter((item) => item.sha256 === expected.sha256).length, 1);
  assert.ok(previewManifest[expected.asset], `Missing preview for ${expected.asset}`);
  assert.match(audit.evidentiaryBoundary, /fields are blank/i);
  assert.match(audit.evidentiaryBoundary, /official November 20, 2017 minutes establish/i);
  assert.deepEqual(audit.queueResolution.closedRequirementIds, []);
});
...
test("batch 73 reuses the exact Wexford County financial audit without treating it as PFAS pathway evidence", async () => {
  const audit = JSON.parse(await readFile(path.join(appDirectory, "batch73-wexford-2018-financial-audit-reconciliation.json"), "utf8"));
  const catalogs = new Map((await loadCatalogs()).map((catalog) => [catalog.name, catalog.rows]));
  const previewManifest = JSON.parse(await readFile(path.join(appDirectory, "first-page-preview-manifest.json"), "utf8"));
  const placement = JSON.parse(await readFile(path.join(publicDirectory, "record-placement-manifest.json"), "utf8"));

  assert.equal(audit.stats.receivedFiles, 1);
  assert.equal(audit.stats.suppliedPathsAvailableAtReview, 1);
  assert.equal(audit.stats.distinctInputHashes, 1);
  assert.equal(audit.stats.pdfPagesRenderedAndReviewed, 112);
  assert.equal(audit.stats.exactExistingRecordsReusedAsCrossReferences, 1);
  assert.equal(audit.stats.recordsAdded, 0);
  assert.equal(audit.stats.timelineEventsAdded, 0);
  assert.equal(audit.stats.evidenceRequirementsClosed, 0);
  ...
  assert.equal(matches[0].size, expected.size);
  assert.equal(matches[0].type, expected.classification);
  assert.match(matches[0].description, /continuing Wexford County Landfill groundwater-remediation obligation/i);
  assert.match(matches[0].description, /not PFAS sampling data or source attribution/i);
  assert.equal(placement.records.filter((item) => item.sha256 === expected.sha256).length, 1);
  assert.ok(previewManifest[expected.localAsset], `Missing preview for ${expected.localAsset}`);

  assert.match(audit.evidentiaryBoundary, /30-year remedial-action/i);
  assert.match(audit.evidentiaryBoundary, /not PFAS sampling data/i);
  assert.match(audit.evidentiaryBoundary, /not.*proof of a source-to-receptor migration pathway/i);
  assert.deepEqual(audit.queueResolution.closedRequirementIds, []);
});
...
test("batch 75 adds the distinct SVN-01952 follow-up while preserving the conditions on final approval", async () => {
  const audit = JSON.parse(await readFile(path.join(appDirectory, "batch75-svn01952-followup-intake-audit.json"), "utf8"));
  const catalog = JSON.parse(await readFile(path.join(appDirectory, "compliance-documents.json"), "utf8"));
  const previewManifest = JSON.parse(await readFile(path.join(appDirectory, "first-page-preview-manifest.json"), "utf8"));
  const placement = JSON.parse(await readFile(path.join(publicDirectory, "record-placement-manifest.json"), "utf8"));
  const pageSource = await readFile(path.join(appDirectory, "page.tsx"), "utf8");
  ...
  const expected = audit.addedRecord;
  const row = catalog.find((item) => item.id === expected.recordId);
  assert.ok(row);
  assert.equal(row.name, expected.canonicalName);
  assert.equal(row.url, expected.asset);
  ...
  assert.equal(row.size, expected.size);
  assert.equal(row.pages, expected.pages);
  assert.equal(row.type, expected.classification);
  assert.equal(catalog.filter((item) => item.sha256 === expected.sha256).length, 1);
  assert.match(row.description, /0\.6 µg\/L/);
  assert.match(row.description, /17 µg\/L/);
  assert.match(row.description, /1,737 µg\/L/);
  assert.match(row.description, /before final approval/i);
  assert.match(row.description, /did not include worker-health-and-safety compliance/i);
  ...
  assert.match(pageSource, /EGLE finds three proposed toxic local limits approvable/);
  assert.match(pageSource, /0\.6 µg\/L for benzidine/);
  assert.match(pageSource, /17 µg\/L for beryllium/);
  assert.match(pageSource, /1,737 µg\/L for phenol/);
  assert.match(pageSource, /not final approval/i);
  assert.match(pageSource, /December 31, 2026/);
  assert.match(pageSource, /did not include worker-health-and-safety compliance/);
  assert.match(audit.evidentiaryBoundary, /not final approval/i);
  assert.match(audit.evidentiaryBoundary, /not.*closure of SVN-01952/i);
  assert.match(audit.evidentiaryBoundary, /not.*PFAS analytical evidence/i);
  assert.deepEqual(audit.queueResolution.closedRequirementIds, []);
});

test("batch 76 preserves two distinct native Word sources and reuses fourteen exact records", async () => {
  const audit = JSON.parse(await readFile(path.join(appDirectory, "batch76-native-word-and-exact-reconciliation-audit.json"), "utf8"));
  const catalogs = new Map((await loadCatalogs()).map((catalog) => [catalog.name, catalog.rows]));
  const placement = JSON.parse(await readFile(path.join(publicDirectory, "record-placement-manifest.json"), "utf8"));

  assert.equal(audit.stats.receivedFiles, 16);
  assert.equal(audit.stats.distinctInputHashes, 16);
  assert.equal(audit.stats.exactExistingRecordsReusedAsCrossReferences, 14);
  assert.equal(audit.stats.exactExistingPdfPagesPreviouslyVerified, 122);
  assert.equal(audit.stats.nativeSourceRecordsAdded, 2);
  assert.equal(audit.stats.wordDocumentsRendered, 2);
  assert.equal(audit.stats.wordPagesRendered, 2);
  assert.equal(audit.stats.documentsWithVbaProjects, 0);
  assert.equal(audit.stats.timelineEventsAdded, 0);
  assert.equal(audit.stats.evidenceRequirementsClosed, 0);
  assert.equal(audit.stats.hashFailures, 0);
  assert.equal(audit.stats.unreadableRecords, 0);
  ...
  for (const expected of audit.addedRecords) {
    ...
    assert.equal(row.url, expected.asset);
    assert.equal(row.size, expected.size);
    assert.equal(row.format, expected.format);
    assert.match(row.description, /PFAS/i);
    const bytes = await readFile(path.join(publicDirectory, expected.asset.replace(/^\//, "")));
    assert.equal(bytes.length, expected.size);
    assert.equal(createHash("sha256").update(bytes).digest("hex"), expected.sha256);
    const placed = placement.records.filter((item) => item.sha256 === expected.sha256);
  ...
  const termination = catalogs.get("reference-documents.json").find((item) => item.id === "131-0805573f14c9");
  const fieldSheet = catalogs.get("reference-documents.json").find((item) => item.id === "132-969c9e8282e7");
  assert.match(termination.description, /letter body matches the indexed official PDF/i);
  assert.match(fieldSheet.description, /distinct from the indexed March 12, 2007 field sheet/i);
  assert.match(fieldSheet.description, /no recorded sample times, analytical results or PFAS analytes/i);

  const terminationBytes = await readFile(path.join(publicDirectory, "reference-data", "131-0805573f14c9.doc"));
  const fieldSheetBytes = await readFile(path.join(publicDirectory, "reference-data", "132-969c9e8282e7.docx"));
  assert.equal(terminationBytes.subarray(0, 8).toString("hex"), "d0cf11e0a1b11ae1");
  assert.equal(fieldSheetBytes.subarray(0, 2).toString("hex"), "504b");
  ...
  assert.equal(referenceAudit.sourceFileCount, referenceAudit.publishedFileCount + referenceAudit.excludedFileCount);
  assert.equal(referenceAudit.includedBytes, referenceCatalog.reduce((sum, item) => sum + item.size, 0));
  assert.match(audit.evidentiaryBoundary, /Neither new record is PFAS analytical evidence/i);
  assert.deepEqual(audit.queueResolution.closedRequirementIds, []);
});
...
test("batch 79 preserves two native compliance sources and reuses eleven exact records", async () => {
  const audit = JSON.parse(await readFile(path.join(appDirectory, "batch79-native-compliance-and-exact-reconciliation-audit.json"), "utf8"));
  const catalogs = new Map((await loadCatalogs()).map((catalog) => [catalog.name, catalog.rows]));
  const placement = JSON.parse(await readFile(path.join(publicDirectory, "record-placement-manifest.json"), "utf8"));

  assert.equal(audit.stats.receivedFiles, 13);
  assert.equal(audit.stats.distinctInputHashes, 13);
  assert.equal(audit.stats.pdfPagesReviewed, 355);
  assert.equal(audit.stats.exactExistingRecordsReusedAsCrossReferences, 11);
  assert.equal(audit.stats.nativeSourceRecordsAdded, 2);
  assert.equal(audit.stats.wordDocumentsRendered, 2);
  assert.equal(audit.stats.wordPagesRendered, 8);
  assert.equal(audit.stats.documentsWithVbaProjects, 0);
  assert.equal(audit.stats.timelineEventsAdded, 0);
  assert.equal(audit.stats.evidenceRequirementsClosed, 0);
  assert.equal(audit.stats.hashFailures, 0);
  ...
  for (const expected of audit.addedRecords) {
    ...
    assert.equal(matches[0].url, expected.asset);
    assert.equal(matches[0].size, expected.size);
    assert.equal(matches[0].format, expected.format);
    const bytes = await readFile(path.join(publicDirectory, expected.asset.replace(/^\//, "")));
    assert.equal(bytes.length, expected.size);
    assert.equal(createHash("sha256").update(bytes).digest("hex"), expected.sha256);
  ...
  const compliance = catalogs.get("reference-documents.json").find((item) => item.id === "133-730dad67c05a");
  const preInspection = catalogs.get("reference-documents.json").find((item) => item.id === "134-89af44caa00a");
  assert.match(compliance.description, /missing total-mercury Pollution Minimization Plan/i);
  assert.match(compliance.description, /contains no PFAS result/i);
  assert.match(preInspection.description, /accepted hauled county-landfill leachate/i);
  assert.match(preInspection.description, /contains no PFAS analytical result/i);
  ...
  assert.equal(referenceAudit.sourceFileCount, referenceAudit.publishedFileCount + referenceAudit.excludedFileCount);
  assert.equal(referenceAudit.includedBytes, referenceCatalog.reduce((sum, item) => sum + item.size, 0));
  assert.match(audit.evidentiaryBoundary, /do not provide PFAS results/i);
  assert.deepEqual(audit.queueResolution.closedRequirementIds, []);
});
...
test("batch 90 reuses the exact Wexford Executive PFAS packet without overstating the proposed disposal", async () => {
  const audit = JSON.parse(await readFile(path.join(appDirectory, "batch90-wexford-executive-pfas-packet-cross-reference-audit.json"), "utf8"));
  const catalogs = new Map((await loadCatalogs()).map((catalog) => [catalog.name, catalog.rows]));
  const placement = JSON.parse(await readFile(path.join(publicDirectory, "record-placement-manifest.json"), "utf8"));
  ...
  assert.equal(audit.stats.pdfPagesReviewed, 15);
  assert.equal(audit.stats.embeddedTextPages, 15);
  assert.equal(audit.stats.pagesOCRCheckedOnGPU, 0);
  assert.equal(audit.stats.manualReviewPages, 0);
  assert.equal(audit.stats.exactExistingRecordsReusedAsCrossReferences, 1);
  assert.equal(audit.stats.recordsAdded, 0);
  assert.equal(audit.stats.timelineEventsAdded, 0);
  ...
  const placed = placement.records.filter((item) => item.sha256 === expected.sha256);
  assert.equal(placed.length, 1);
  assert.equal(placed[0].archiveId, "pfas");
  assert.match(expected.boundary, /request and recommendation for discussion, not final approval/i);
  assert.match(audit.evidentiaryBoundary, /does not establish that the proposed soil was accepted or disposed/i);
  assert.deepEqual(audit.queueResolution.closedRequirementIds, []);
});
...
test("batch 92 reuses the exact Cadillac injection-well resolution while preserving its unsigned-form boundary", async () => {
  const audit = JSON.parse(await readFile(path.join(appDirectory, "batch92-cadillac-injection-well-resolution-cross-reference-audit.json"), "utf8"));
  const catalogs = new Map((await loadCatalogs()).map((catalog) => [catalog.name, catalog.rows]));
  const placement = JSON.parse(await readFile(path.join(publicDirectory, "record-placement-manifest.json"), "utf8"));
  ...
  assert.equal(audit.stats.embeddedTextPages, 5);
  assert.equal(audit.stats.pagesOCRCheckedOnGPU, 0);
  assert.equal(audit.stats.ocrPagesWithText, 0);
  assert.equal(audit.stats.manualReviewPages, 0);
  assert.equal(audit.stats.exactExistingRecordsReusedAsCrossReferences, 1);
  assert.equal(audit.stats.recordsAdded, 0);
  assert.equal(audit.stats.timelineEventsAdded, 0);
  ...
  assert.equal(placed.length, 1);
  assert.equal(placed[0].archiveId, "supplemental");
  assert.match(expected.finding, /more than 20 years/i);
  assert.match(expected.finding, /14 million gallons in 2016/i);
  assert.match(expected.finding, /20 million gallons annually/i);
  assert.match(expected.boundary, /unsigned and unexecuted/i);
  assert.match(expected.boundary, /separate official Council minutes establish adoption of motion 2017-232/i);
  assert.match(expected.boundary, /contains no PFAS analysis or result/i);
  assert.match(audit.queueDecision, /No evidence-request item is closed/i);
  assert.match(audit.evidentiaryBoundary, /does not by itself prove formal adoption/i);
  assert.deepEqual(audit.queueResolution.closedRequirementIds, []);
});
...
test("batch 94 reconciles the MiEnviro index, reuses 22 exact records and adds only the distinct UV correspondence", async () => {
  const audit = JSON.parse(await readFile(path.join(appDirectory, "batch94-mienviro-indexed-npdes-correspondence-intake-audit.json"), "utf8"));
  const catalogs = new Map((await loadCatalogs()).map((catalog) => [catalog.name, catalog.rows]));
  const placement = JSON.parse(await readFile(path.join(publicDirectory, "record-placement-manifest.json"), "utf8"));
  const previews = JSON.parse(await readFile(path.join(appDirectory, "first-page-preview-manifest.json"), "utf8"));

  assert.equal(audit.stats.suppliedPdfs, 23);
  assert.equal(audit.stats.distinctInputHashes, 23);
  assert.equal(audit.stats.suppliedIndexRows, 35);
  assert.equal(audit.stats.suppliedIndexColumns, 10);
  assert.equal(audit.stats.suppliedPdfPagesReviewed, 300);
  assert.equal(audit.stats.embeddedTextPages, 264);
  assert.equal(audit.stats.pagesOCRCheckedOnGPU, 36);
  assert.equal(audit.stats.ocrPagesWithText, 36);
  assert.equal(audit.stats.manualReviewPages, 0);
  assert.equal(audit.stats.indexRowsMatchedToSuppliedPdfs, 23);
  assert.equal(audit.stats.indexHashMismatches, 0);
  assert.equal(audit.stats.indexSizeMismatches, 0);
  assert.equal(audit.stats.exactExistingRecordsReusedAsCrossReferences, 22);
  assert.equal(audit.stats.recordsAdded, 1);
  assert.equal(audit.stats.addedPdfPages, 3);
  assert.equal(audit.stats.timelineEventsAdded, 0);
  assert.equal(audit.stats.requirementsClosed, 0);
  assert.equal(audit.stats.hashFailures, 0);
  assert.equal(audit.stats.pageCountFailures, 0);
  assert.equal(audit.stats.unreadableRecords, 0);
  assert.equal(audit.sourceIndex.sha256, "e33cb0f0fd5f2b26faf4ff72d41962d498e2f6779b0763764d6ac743c93fb8b4");
  assert.equal(audit.sourceIndex.errorRows, 0);
  assert.equal(audit.exactCrossReferences.length, 22);

  for (const expected of audit.exactCrossReferences) {
    const catalog = catalogs.get(expected.catalog);
    const matches = catalog.filter((item) => item.sha256 === expected.sha256);
    assert.equal(matches.length, 1, `${expected.suppliedName} must resolve to one catalog record`);
    assert.equal(matches[0].id, expected.recordId);
    assert.equal(matches[0].size, expected.size);
    assert.equal(matches[0].pages, expected.pages);
    const acrossCatalogs = [...catalogs.values()].flat().filter((item) => item.sha256 === expected.sha256);
    assert.equal(acrossCatalogs.length, 1, `${expected.suppliedName} must not be republished`);
    const placed = placement.records.filter((item) => item.sha256 === expected.sha256);
    assert.equal(placed.length, 1);
    assert.equal(placed[0].archiveId, expected.archiveId);
  }

  const added = audit.addedRecords[0];
  const addedMatches = catalogs.get(added.catalog).filter((item) => item.sha256 === added.sha256);
  assert.equal(addedMatches.length, 1);
  assert.equal(addedMatches[0].id, added.recordId);
  assert.equal(addedMatches[0].pages, 3);
  assert.equal(addedMatches[0].size, added.size);
  assert.match(addedMatches[0].description, /UV had completely replaced chlorine/i);
  assert.match(addedMatches[0].description, /contains no PFAS analytical result/i);
  const sourcePath = path.join(publicDirectory, ...added.asset.slice(1).split("/"));
  const sourceBytes = await readFile(sourcePath);
  assert.equal(createHash("sha256").update(sourceBytes).digest("hex"), added.sha256);
  const previewPath = previews[added.asset];
  assert.ok(previewPath);
  assert.ok((await stat(path.join(publicDirectory, ...previewPath.slice(1).split("/")))).size > 0);
  const placedAdded = placement.records.filter((item) => item.sha256 === added.sha256);
  assert.equal(placedAdded.length, 1);
  assert.equal(placedAdded[0].archiveId, "correspondence");
  assert.match(added.finding, /sulfur-dioxide equipment had been removed/i);
  assert.match(added.boundary, /no PFAS analytical result/i);
  assert.match(audit.queueDecision, /No evidence-request item is closed/i);
  assert.deepEqual(audit.queueResolution.closedRequirementIds, []);
});
...
test("laboratory archive audit preserves revisions and suppresses only verified wrapper copies", async () => {
  ...
  assert.equal(audit.stats.finalDistinctRecords, 99);
