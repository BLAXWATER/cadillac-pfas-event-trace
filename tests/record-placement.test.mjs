...
test("every source record is assigned to exactly one intended visible archive block", async () => {
  ...
  assert.equal(manifest.recordCount, 1613);
  assert.equal(new Set(manifest.records.map((record) => record.id)).size, 1613);
  assert.equal(manifest.archives.reduce((sum, archive) => sum + archive.count, 0), 1613);
});
