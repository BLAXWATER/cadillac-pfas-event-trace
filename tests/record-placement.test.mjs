...
test("every source record is assigned to exactly one intended visible archive block", async () => {
  ...
  assert.equal(manifest.recordCount, 1557);
  assert.equal(new Set(manifest.records.map((record) => record.id)).size, 1557);
  assert.equal(manifest.archives.reduce((sum, archive) => sum + archive.count, 0), 1557);
});
