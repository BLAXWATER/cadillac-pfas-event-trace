...
test("renders stable site metadata and source policy", async () => {
  ...
  assert.match(html, /2018: jump to 17 events/i);
  assert.match(html, /<strong>2018<\/strong><span>17(?:<!-- -->\s*)+events<\/span>/i);
  ...
  assert.equal((html.match(/class="trace-row"/gi) ?? []).length, 114);
