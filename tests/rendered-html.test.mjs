...
test("renders stable site metadata and source policy", async () => {
  ...
  assert.match(html, /2018: jump to 12 events/i);
  assert.match(html, /<strong>2018<\/strong><span>12(?:<!-- -->\s*)+events<\/span>/i);
  ...
  assert.equal((html.match(/class="trace-row"/gi) ?? []).length, 66);
