...
const siteUrl = process.argv[2] ?? process.env.SITE_URL ?? defaultSiteUrl;
const target = new URL("record-placement-manifest.json", siteUrl);
...
console.log(`Public placement integrity passed: ${publicManifest.recordCount} records in ${publicManifest.archiveCount} archive blocks at ${target.origin}.`);
